...
const poisk = ['iphone','samsung','huawei','xiaomi','airpods','macbook','scontrino','mediaworld'];  // строка поиска 
const pag = 17;           // сколько страниц обьявлений обработать 
...
const proxy = 'socks5://65.20.96.2:1080'; // socks5://login:pass@1.1.1.1:port
...
const start2 = async (array) => {
  ...
  for(let a = 0; a< array.length; a++)
  {
    try {
      ...
      const response = await axiosInstance.get(array[a]);
      const html = response.data;
      const $ = cheerio.load(html);
      ...
      if (sliderContainer.length > 0) {
        ...
        console.log('Не удалось найти тег <ul> с классом "slider-container"','>>> ',array[a]);
      }
    } catch (error) {
      console.error('Ошибка при запросе:   ',a,array[a], error);
    }
  }
